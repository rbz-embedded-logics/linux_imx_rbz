...
/**
 * __hw_protection_shutdown - Trigger an emergency system poweroff or reboot
 *
 * @reason:		Reason of emergency shutdown or reboot to be printed.
 * @ms_until_forced:	Time to wait for orderly shutdown or rebootbefore 
 *                      tiggering it. Negative value disables the forced
 *			            shutdown or reboot.
 * @shutdown:           If true, indicates that a shutdown will happend
 *                      after the critical temperature is reached.
 *                      If false, indicates that a reboot will happend
 *                      after the critical temperature is reached.
 *
 * Initiate an emergency system shutdown or reboot in order to protect hardware from
 * further damage. Usage examples include a thermal protection or a voltage or
 * current regulator failures.
 * NOTE: The request is ignored if protection shutdown or reboot is already pending even
 * if the previous request has given a large timeout for forced shutdown/reboot.
